...
contract ColorMarketplace is ReentrancyGuard {
    ...
    /**
     * @dev Checks the purchase requirements for a listing.
     * @param listing The storage reference to the listing.
     * @param amount The amount of tokens being purchased.
     * 
     * Requirements:
     * - The listing's token type must be either ERC721 or ERC1155.
     * - If the token type is ERC721:
     *   - The market must own the token.
     *   - The amount must be 1 (partial ERC721 tokens are not allowed).
     * - If the token type is ERC1155:
     *   - The market must have enough tokens to sell.
     * - If the token type is neither ERC721 nor ERC1155, reverts with an error message.
     */
